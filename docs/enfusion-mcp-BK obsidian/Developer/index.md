...
- **data/wiki/ - wiki_search / wiki_read source:** export.xml (MediaWiki export-0.11 XML of the Bohemia Interactive Community wiki, ~5.3 MB) and pages.json (258 articles as {title, source: bistudio-wiki, content, url}); a `patterns/` subdirectory exists but is empty.
